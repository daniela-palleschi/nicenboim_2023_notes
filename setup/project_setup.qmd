...
# Project set-up {.unnumbered}

This script can be run prior to starting the book "An Introduction to Bayesian Data Analysis for Cognitive Science" by
Bruno Nicenboim, Daniel Schad, and Shravan Vasishth [@nicenboim_introduction_nodate]. The code is based on the section [Software needed](https://vasishth.github.io/bayescogsci/book/software-needed.html).
...
If you want to reproduce the code contained in this book:
...
Changed computers for a new job (from a 2023 MacBook Pro with M2 Silicon chip to 2019 MacBook Pro with i9 Intel processor). This caused problems when installing some packages, namely `VGAM`. I deactivated {renv} (`renv::deactivate()`), then downloaded gfortran6.1.pkg at [https://cran.r-project.org/bin/macosx/tools/](https://cran.r-project.org/bin/macosx/tools/), and re-activated renv and reinstalled everything. It took some fiddling but it worked in the end.
